...
def play_roulette(fib_sequence, games):
    ...
    for i in range(1, games + 1):
        ...
        balance_history.append((i, total_profit - total_loss, total_win, total_loss, spin_result))
...
for x, y, _, _, spin_result in balance_history:
    if y < 0:
...
print("Hand Number\tTotal Win\tTotal Loss\tBalance\tWinning Number")
for item in balance_history:
    print("{:12}\t{:10}\t{:10}\t{:10}\t{:14}".format(item[0], item[2], item[3], item[1], item[4]))

# Export the balance history to a text file
with open('balance_history.txt', 'w') as f:
    for item in balance_history:
        f.write("{},{},{},{},{}\n".format(item[0], item[2], item[3], item[1], item[4]))
import random
import matplotlib.pyplot as plt
...
# Print the balance history to the console
print("Balance History:")
print("Hand Number\tTotal Win\tTotal Loss\tBalance\tWinning Number\tTotal Bet")
for item in balance_history:
